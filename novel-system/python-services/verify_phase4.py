...
def check_phase4_structure():
    """检查Phase 4目录结构"""
    print("=" * 70)
    print("Phase 4 代码结构验证")
    print("=" * 70)
    ...
    results.append(check_file_exists(
        base_path / "schemas/outline_schemas.py",
        "大纲Schema定义"
    ))
    ...
    results.append(check_file_exists(
        base_path / "agents/outline_generator_agent.py",
        "OutlineGeneratorAgent实现"
    ))
...
def check_file_content():
    ...
    # 检查outline_schemas.py的主要类
    print("\n1. 检查outline_schemas.py:")
    schemas_file = base_path / "schemas/outline_schemas.py"
    ...
    # 检查agents/__init__.py是否包含OutlineGeneratorAgent
    print("\n2. 检查agents/__init__.py:")
    agents_init = base_path / "agents/__init__.py"
    if agents_init.exists():
        content = agents_init.read_text()
        exists = "OutlineGeneratorAgent" in content
        status = "✓" if exists else "✗"
        print(f"  {status} OutlineGeneratorAgent已导出")
        checks.append(exists)
    else:
        print("  ✗ 文件不存在")
        checks.append(False)

    # 检查agent_routes.py是否注册了outline_generation
    print("\n3. 检查agent_routes.py:")
    routes_file = base_path / "api/routes/agent_routes.py"
    if routes_file.exists():
        content = routes_file.read_text()
        exists = "outline_generation" in content
        status = "✓" if exists else "✗"
        print(f"  {status} outline_generation已注册")
        checks.append(exists)
    else:
        print("  ✗ 文件不存在")
        checks.append(False)
...
def count_lines():
    """统计代码行数"""
    print("\n" + "=" * 70)
    print("Phase 4 代码行数统计")
    print("=" * 70)

    base_path = Path(__file__).parent
    files = [
        ("schemas/outline_schemas.py", "大纲Schema定义"),
        ("agents/outline_generator_agent.py", "OutlineGeneratorAgent"),
    ]
    ...
    print(f"\n  {'Phase 4 新增代码总计':30s} : {total_lines:4d} 行")
    return total_lines
...
def check_agent_functionality():
    ...
    base_path = Path(__file__).parent
    agent_file = base_path / "agents/outline_generator_agent.py"

    if not agent_file.exists():
        print("  ✗ OutlineGeneratorAgent文件不存在")
        return False
    ...
    required_methods = [
        ...
        "_generate_world_settings",
        "_generate_volumes_plan",
        "_generate_volume_outline",
        "_validate_outline",
        "_save_outline"
    ]
...
def main():
    ...
    # 检查文件结构
    structure_results = check_phase4_structure()
