...
class RetrievalIndexAgent(BaseAgent):
    ...
    async def run(self, request: AgentRequest) -> AgentResponse:
        ...
        try:
            ...
            rerank_application = await self._apply_gateway_rerank(
                request,
                builder.project_root,
                query,
                retrieval,
    ...
    def _rerank_backend(self, request: AgentRequest, project_root: Path) -> str:
        explicit = (
            request.parameters.get("rerank_backend")
            or request.parameters.get("rerankBackend")
            or request.parameters.get("reranker")
            or request.parameters.get("reranker_backend")
            or request.parameters.get("rerankerBackend")
        )
        if explicit is not None:
            return self._normalize_rerank_backend(explicit, request)
        ...
        if configured is not None:
            return self._normalize_rerank_backend(configured, request)
        return "rules"

    def _normalize_rerank_backend(self, value: Any, request: AgentRequest) -> str:
        ...
        if text in {"auto", "default"}:
            return "llm_gateway" if self._has_external_rerank_profile(request.model_profile_id) else "rules"
        return "rules"

    def _has_external_rerank_profile(self, model_profile_id: Optional[str]) -> bool:
        if not model_profile_id:
            return False
        try:
            config = self.llm_client._select_profile_model(model_profile_id, "rerank")
        except Exception:
            return False
        return bool(config and not config.get("mock"))

    async def _apply_gateway_rerank(
            self,
            request: AgentRequest,
            project_root: Path,
            query: str,
        ...
        requested = self._rerank_backend(request, project_root)
        ...
        if requested in {"rules", "none"}:
            if requested == "none":
                base_summary.update({"active": "rules", "status": "rules_fallback", "reason": "use_rerank_kept_rule_results"})
        ...
        try:
            response = await self.llm_client.rerank(
                query,
                documents,
            ...
            ranked = self._merge_gateway_rerank(candidates, response.get("results") or [], top_k)
            if not ranked:
    ...
    def _merge_gateway_rerank(
        ...
        for rank, item in enumerate(ranked, start=1):
            ...
            model_score = self._float(item.get("score"), 0.0)
